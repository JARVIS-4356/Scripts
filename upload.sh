...
[7] Sourceforge [Key]
"
read -p "Please enter your choice: " UP
...
echo -e "Started uploading file to DevUploads..."
bash <(curl -s https://devuploads.com/upload.sh) -f $FP -k $KEY
fi

if [ $UP == 3 ]; then
read -p "Please enter Pixel Drain key: " KEY
echo -e "Started uploading file to PixelDrain..."
curl -T $FP -u ":$KEY" https://pixeldrain.com/api/file/
fi

if [ $UP == 4 ]; then
echo -e "Started uploading file to Temp..."
curl -T $FP temp.sh
fi

if [ $UP == 5 ]; then
echo -e "Started uploading file to Gofile..."
...
if [ $UP == 7 ]; then
echo -e "Start uploading file to Sourceforge..."
